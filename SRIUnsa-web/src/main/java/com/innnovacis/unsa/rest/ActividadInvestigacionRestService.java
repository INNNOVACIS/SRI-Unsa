...
public class ActividadInvestigacionRestService {
    ...
    public Response GetAll(){
        ...
        try {
            lstActividadInvestigacion = investigacionBusiness.GetAll();
...
//    }
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces(MediaType.APPLICATION_JSON)
    public Response UpdateActividadInvestigacion(SRIActividadInvestigacion investigacion){
        Response.ResponseBuilder builder = null;
        Map<String, Object> respuesta = new HashMap<>();
        boolean body = false;
        try {
            body = investigacionBusiness.Update(investigacion);
            respuesta.put("body", body);
            builder = Response.status(Response.Status.OK).entity(respuesta);
            log.log(Level.INFO, "ActividadInvestigacionUpdate - Success : {0}", investigacion.toString());
        } catch(Exception ex) {
            builder = Response.status(Response.Status.INTERNAL_SERVER_ERROR).entity(ex.getMessage());
    ...
    public Response DeleteActividadInvestigacion(SRIActividadInvestigacion investigacion){
        ...
        boolean body = false;
        try {
            body = investigacionBusiness.Delete(investigacion);
            respuesta.put("body", body);
            builder = Response.status(Response.Status.OK).entity(respuesta);
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public Response getActividadById(@PathParam("id") int id) {
        Response.ResponseBuilder builder = null;
        Map<String, Object> respuesta = new HashMap<>();
        SRIActividadInvestigacion body = null;
        try {
            body = investigacionBusiness.Get(id);
            respuesta.put("body", body);
            if (body == null) {
                builder = Response.status(Response.Status.NOT_FOUND).entity(respuesta);
                log.log(Level.INFO, "GetActividadById - Nulo : {0}", body.toString());
            } else {
                builder = Response.status(Response.Status.OK).entity(respuesta);
    ...
    public Response RegistrarActividad(SRIActividadGeneral entidad){
        ...
        try {
            body = investigacionBusiness.RegistrarActividad(entidad);
            respuesta.put("body", body);
            builder = Response.status(Response.Status.OK).entity(respuesta);
    ...
    public Response EnviarEmail(SRIActividadGeneral entidad){
        ...
        boolean body = false;
        try {
            body = investigacionBusiness.EnviarEmail(entidad);
            respuesta.put("body", body);
            builder = Response.status(Response.Status.OK).entity(respuesta);
